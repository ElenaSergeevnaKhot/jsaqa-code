...
describe("let's go to the cinema tests", () => {
   ...
   test("The first test'", async () => {
    ...
    test("Open Зал 1", async () => {
    const actual = await getText(page, ".movie-seances__hall-title");
    expect(actual).toContain("Зал 1");
  }, 70000);

  test("Booking for 00.00", async () => {
   const btnSelector = ".movie-seances__time-block";
   await page.waitForSelector(btnSelector, {
      visible: true,
    });
    const actual = await getText(page, ".movie-seances__time-block");
